...
import { collectGroups, useWorkbench, type EditorMode, type GroupNode, type LayoutNode, type SaveState, type WorkbenchTab } from '../workbench'
...
function DocumentPage() {
  ...
  useEffect(() => workbenchRef.current.openDocument(routeDocumentId), [routeDocumentId])
  ...
  return (
    <div className="editor-tabbar" role="tablist" aria-label="Open documents" onPointerDown={() => workbench.setActiveGroup(groupId)}>
      <div className="editor-tabs-scroll">
        {tabs.map((tab) => {
          const session = workbench.sessions[tab.documentId]
          const dirty = session && session.saveState !== 'saved'
          return (
            <div key={tab.documentId} className={activeDocumentId === tab.documentId ? 'editor-tab active' : 'editor-tab'}>
              <button role="tab" aria-selected={activeDocumentId === tab.documentId} title={tab.title} onClick={() => onActivate(tab.documentId)}>
...
function DocumentWorkspace({ initialDocument, showInspector, onDeleted }: { initialDocument: Document; showInspector: boolean; onDeleted: () => void }) {
  ...
  const session = workbench.sessions[documentId]
  const content = session?.content ?? document.content
  const saveState = session?.saveState ?? 'saved'
  const mode = session?.mode ?? 'edit'
  const selection = session?.selection ?? { line: 1, column: 1, selectedCharacters: 0 }
  ...
  const save = useMutation({
    ...
    onMutate: () => workbench.updateSession(documentId, { saveState: navigator.onLine ? 'saving' : 'offline' }),
    onSuccess: (nextDocument, variables) => {
      updateCachedDocument(nextDocument)
      workbench.updateSession(documentId, {
        saveState: content === variables.nextContent ? 'saved' : 'dirty',
        baseRevisionId: nextDocument.current_revision_id,
        content,
      })
    },
    onError: (error) => workbench.updateSession(documentId, { saveState: error instanceof ApiError && error.status === 409 ? 'conflict' : navigator.onLine ? 'failed' : 'offline' }),
  })
  useEffect(() => {
    if (content === document.content || save.isPending || saveState === 'conflict' || !navigator.onLine) return
    const base = session?.baseRevisionId ? { ...document, current_revision_id: session.baseRevisionId } : document
    const timeout = window.setTimeout(() => save.mutate({ base, nextContent: content }), 800)
    return () => window.clearTimeout(timeout)
  }, [content, document, save, saveState, session?.baseRevisionId])
  useEffect(() => {
    const online = () => workbenchRef.current.updateSession(documentId, { saveState: content === document.content ? 'saved' : 'dirty' })
    const offline = () => workbenchRef.current.updateSession(documentId, { saveState: content === document.content ? 'saved' : 'offline' })
    window.addEventListener('online', online)
    window.addEventListener('offline', offline)
    return () => { window.removeEventListener('online', online); window.removeEventListener('offline', offline) }
  }, [content, document.content, documentId])
  useEffect(() => {
  ...
  const materialize = useMutation({ mutationFn: ({ base, path }: { base: Document; path: string }) => api.materializeDocument(base, path), onSuccess: updateCachedDocument })
  const restore = useMutation({
    mutationFn: ({ base, revisionId }: { base: Document; revisionId: string }) => api.restore(base, revisionId),
    onSuccess: (nextDocument) => {
      updateCachedDocument(nextDocument)
      workbench.updateSession(documentId, { content: nextDocument.content, baseRevisionId: nextDocument.current_revision_id, saveState: 'saved', compareFrom: undefined })
    },
  })
  const reloadAfterConflict = async () => {
    const current = await api.getDocument(documentId)
    updateCachedDocument(current)
    workbench.updateSession(documentId, { content: current.content, baseRevisionId: current.current_revision_id, saveState: 'saved' })
  }
  const handleEditorChange = (nextContent: string) => {
    workbench.updateSession(documentId, {
      content: nextContent,
      baseRevisionId: session?.baseRevisionId ?? document.current_revision_id,
      saveState: nextContent === document.content ? 'saved' : saveState === 'conflict' ? 'conflict' : navigator.onLine ? 'dirty' : 'offline',
    })
  }
  const insertLink = () => {
  ...
  return (
    <div className="document-layout tab-document-layout">
      <section className="document-workspace">
        ...
        <DocumentToolbar document={document} content={content} saveState={saveState} mode={mode} onMode={(nextMode) => workbench.updateSession(documentId, { mode: nextMode })} onUpdated={(updated) => { updateCachedDocument(updated); workbench.updateSession(documentId, { content: updated.content, baseRevisionId: updated.current_revision_id, saveState: 'saved' }) }} onDeleted={async () => { await queryClient.invalidateQueries({ queryKey: ['documents'] }); onDeleted() }} />
        ...
        <div className={`editing-surface mode-${mode}`}>
          {mode !== 'preview' && <Suspense fallback={<div className="editor muted">Preparing editor…</div>}><MarkdownEditor ref={editorRef} value={content} onChange={handleEditorChange} onSelectionChange={(nextSelection) => workbench.updateSession(documentId, { selection: nextSelection })} initialViewState={session?.viewState} onViewStateChange={(viewState) => workbench.updateSession(documentId, { viewState })} /></Suspense>}
