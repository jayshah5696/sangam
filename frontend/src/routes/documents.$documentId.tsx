...
function LayoutRenderer({ node, routeDocumentId }: { node: LayoutNode; routeDocumentId: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <LayoutRenderer node={node.first} routeDocumentId={routeDocumentId} />
      </Panel>
      <Separator
      ...
      <Panel
        ...
        minSize={node.direction === 'horizontal' ? '280px' : '220px'}
      >
        <LayoutRenderer node={node.second} routeDocumentId={routeDocumentId} />
      </Panel>
    </PanelGroup>
  )
}
...
function EditorGroupView({ group, routeDocumentId }: { group: GroupNode; routeDocumentId: string }) {
  ...
  const activeDocumentId =
    group.activeTabId ?? (group.id === workbench.activeGroupId ? routeDocumentId : null)
  ...
  return (
    ...
    >
      <TabStrip
        ...
        onSplit={(direction) =>
          workbench.splitGroup(group.id, direction, activeDocumentId ?? routeDocumentId)
        }
...
function TabStrip({
  ...
  return (
    <div
      ...
      onPointerDown={() => workbench.setActiveGroup(groupId)}
    >
      ...
      <details className="tab-actions">
        ...
        <div>
          <button onClick={() => onPin(activeDocumentId)}>
            {tabs.find((tab) => tab.documentId === activeDocumentId)?.pinned ? (
              <PinOff size={13} />
            ) : (
              <Pin size={13} />
            )}{' '}
            {tabs.find((tab) => tab.documentId === activeDocumentId)?.pinned ? 'Unpin tab' : 'Pin tab'}
          </button>
...
function DocumentWorkspace({
  ...
  const restore = useMutation({
    mutationFn: ({ base, revisionId }: { base: Document; revisionId: string }) =>
      api.restore(base, revisionId),
      ...
      {tab === 'history' && (
        <>
          <section className="compare-controls">
            <label>
              ...
              <select
                value={compareFrom ?? ''}
                onChange={(event) => {
                  if (event.target.value) onComparison(event.target.value, compareTo)
                }}
              >
            ...
            <label>
              ...
              <select
                value={compareTo}
                onChange={(event) => {
                  if (compareFrom) onComparison(compareFrom, event.target.value)
                }}
              >
                {history.map((revision) => (
                  <option key={revision.revision_id} value={revision.revision_id}>
            ...
            {fromRevision && toRevision && fromRevision.revision_id !== toRevision.revision_id && (
              <button onClick={onCloseComparison}>Close comparison</button>
            )}
          </section>
          {fromRevision && toRevision && fromRevision.revision_id !== toRevision.revision_id && (
          ...
          <HistoryList
            ...
            busy={restoring || saveState !== 'saved'}
            onCompare={(revisionId) => onComparison(revisionId, document.current_revision_id)}
            onCopy={onCopy}
            onRestore={onRestore}
          />
        </>
