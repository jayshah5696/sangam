...
export function FileExplorerPanel({ onSearch }: { onSearch: () => void }) {
  ...
  const tree = useMemo(() => buildWorkspaceTree(documents.data ?? [], folders.data ?? []), [documents.data, folders.data])
  ...
  const create = useMutation({
    mutationFn: async ({ mode, name }: { mode: Exclude<CreateMode, null>; name: string }) => {
      if (mode.kind === 'folder') return { folder: await api.createFolder(joinWorkspacePath(mode.parentPath, name)) }
  ...
  const move = useMutation({
    ...
    onMutate: async ({ document, folderPath }) => {
      ...
      const filename = document.path?.split('/').at(-1)
      if (filename) queryClient.setQueryData<Document[]>(['documents', 'explorer'], (current) => current?.map((candidate) => candidate.document_id === document.document_id ? { ...candidate, path: joinWorkspacePath(folderPath, filename) } : candidate))
      return { previous }
    },
  ...
  const onTreeKeyDown = (event: KeyboardEvent<HTMLElement>) => {
    ...
    const focusNode = (nodeId: string | null) => {
      ...
      requestAnimationFrame(() => treeRef.current?.querySelector<HTMLElement>(`[data-node-id="${CSS.escape(nodeId)}"]`)?.focus())
    }
    ...
    if (event.key === 'ArrowLeft') {
      ...
      if (node.type === 'folder' && expanded.has(node.path)) {
        setExpanded((currentSet) => { const next = new Set(currentSet); next.delete(node.path); return next })
  ...
  const actions: ExplorerActions = {
    ...
    toggle: (path) => setExpanded((current) => {
    ...
    startRename,
    setRenameValue,
    commitRename: (target) => { if (renameValue.trim()) rename.mutate({ node: target, value: renameValue.trim() }) },
    cancelRename: () => setRenamingId(null),
    create: (kind, parentPath) => setCreateMode({ kind, parentPath }),
    duplicate: (target) => duplicate.mutate(target.document),
    trash: (target) => { if (window.confirm(`Move “${target.document.title}” to trash?`)) remove.mutate(target.document) },
    dropDocument,
  }
  ...
  return (
    ...
    <div className="sidebar-content file-explorer-panel">
      ...
      <nav ref={treeRef} role="tree" aria-label="Files" className="explorer-tree" onKeyDown={onTreeKeyDown} onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropDocument(event, '')}>
        {tree.map((node) => <ExplorerNodeView key={node.id} node={node} level={1} />)}
...
function ExplorerNodeView({ node, level }: {
  node: ExplorerNode
  level: number
}) {
  ...
  if (node.type === 'folder') {
    return (
      <div role="treeitem" aria-level={level} aria-expanded={isExpanded} aria-selected={actions.selectedId === node.id} className="explorer-branch">
        <div className={actions.selectedId === node.id ? 'explorer-row selected' : 'explorer-row'} style={{ paddingLeft: 5 + (level - 1) * 13 }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); actions.dropDocument(event, node.virtual ? '' : node.path) }}>
          <button data-tree-row data-node-id={node.id} tabIndex={actions.selectedId === node.id ? 0 : -1} className="explorer-label" onClick={() => { actions.select(node.id); actions.toggle(node.path) }}>
          ...
          {!node.virtual && <details className="tree-menu"><summary aria-label={`Actions for ${node.name}`}><MoreHorizontal size={13} /></summary><div><button onClick={() => actions.create('file', node.path)}><FilePlus2 size={12} />New file</button><button onClick={() => actions.create('folder', node.path)}><FolderPlus size={12} />New folder</button></div></details>}
        </div>
        {isExpanded && <div role="group">{node.children.map((child) => <ExplorerNodeView key={child.id} node={child} level={level + 1} />)}</div>}
      </div>
    )
  }
  return (
    <div role="treeitem" aria-level={level} aria-selected={actions.selectedId === node.id} className={actions.selectedId === node.id ? 'explorer-row selected' : 'explorer-row'} style={{ paddingLeft: 20 + (level - 1) * 13 }} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-sangam-document', node.document.document_id) }}>
      {actions.renamingId === node.id ? <form className="tree-rename" onSubmit={(event) => { event.preventDefault(); actions.commitRename(node) }}><input autoFocus aria-label={`Rename ${node.name}`} value={actions.renameValue} onChange={(event) => actions.setRenameValue(event.target.value)} onBlur={() => actions.commitRename(node)} onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); actions.cancelRename() } }} /></form> : <button data-tree-row data-node-id={node.id} tabIndex={actions.selectedId === node.id ? 0 : -1} className="explorer-label" onClick={() => { actions.select(node.id); actions.open(node) }} onDoubleClick={() => actions.startRename(node)}><FileText size={13} /><span>{node.name}</span></button>}
      <details className="tree-menu"><summary aria-label={`Actions for ${node.name}`}><MoreHorizontal size={13} /></summary><div><button onClick={() => actions.openToSide(node)}><PanelRightOpen size={12} />Open to side</button><button onClick={() => actions.startRename(node)}><Pencil size={12} />Rename</button><button onClick={() => actions.duplicate(node)}><Copy size={12} />Duplicate</button><button className="danger" onClick={() => actions.trash(node)}><Trash2 size={12} />Move to trash</button></div></details>
    </div>
  )
}
